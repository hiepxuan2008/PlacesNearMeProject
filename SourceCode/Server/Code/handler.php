...
switch ($cid) {
    ...
    case "placeNearMeByProductName": {
        placeNearMeByProductName();
        break;
    }
    default:
        break;
...
function placeNearMeByProductName() {
    global $response, $db_com;
    if (check_miss_required_params(["lat", "lng", "search"]) == 1) return;
    
    $lat = $_POST["lat"];
    $lng = $_POST["lng"];
    $search = $_POST["search"];
    
    $res = $db_com->placeNearMeByProductName($lat, $lng, $search);
...
function notify_post_status($own_user_id, $google_place_id) {
    ...
    $title = "PlaceShare";
    $message = "Trần Tuấn Tú has just posted a new feeling to the place that you are following!";
